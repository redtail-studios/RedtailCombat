...
from config import (GENRES, ACTIVE_GENRES, YOUTUBE_API_KEY, YT_QUERIES_COMMON,
                    YT_VIDEOS_PER_Q, YT_COMMENTS_PER, get_year_dir)
...
def run(year: int | None = None, log=print) -> list:
    ...
    queries = [(q, "general") for q in YT_QUERIES_COMMON]
    queries += [(GENRES[g]["yt_query"], g) for g in ACTIVE_GENRES]

    log(f"[youtube] {len(queries)} queries (year={year})")
    records = []
    for q, genre in queries:
        for vid, title in _search_videos(q, year):
            comments = _comments(vid)
            records.append({
                "source": "youtube", "query": q, "genre": genre, "video_id": vid,
