...
from config import GENRES, ACTIVE_GENRES, TREND_TERMS_COMMON, get_year_dir
...
def run(year: int | None = None, log=print) -> list:
    ...
    # (term, genre tag) pairs — common terms tag "general", each active genre
    # additionally contributes its own term tagged with that genre.
    terms = [(t, "general") for t in TREND_TERMS_COMMON]
    terms += [(GENRES[g]["trend_term"], g) for g in ACTIVE_GENRES]

    y = year or 2026
    timeframe = f"{y}-01-01 {y}-12-31"
    log(f"[googletrends] {len(terms)} terms over {timeframe}")
    try:
    ...
    for term, genre in terms:
        ...
        records.append({
            "source": "googletrends", "term": term, "genre": genre, "avg_interest": avg,
