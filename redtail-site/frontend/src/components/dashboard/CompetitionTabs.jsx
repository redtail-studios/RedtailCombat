  ...
  return <>
    ...
    <div id="competition-panel" role="tabpanel" aria-labelledby={`tab-${tab}`} className={`grid ${tab==='Competition'?'lg:grid-cols-[minmax(0,1fr)_280px]':'lg:grid-cols-[minmax(0,1.65fr)_minmax(260px,1fr)]'} gap-5`}>
      <section className="bg-panel border border-white/10 pixel-clip overflow-hidden"><div className="p-5 sm:p-6 border-b border-white/10"><p className="font-mono text-[10px] text-moss tracking-widest mb-3">01 / {tab.toUpperCase()}</p><h2 className="font-pixel text-base sm:text-xl leading-relaxed">{title}</h2><p className="font-mono text-xs text-platinum/50 mt-3 leading-relaxed">{tab==='Genre'?'Your game, connected to its five closest genres. Shorter lines indicate stronger similarity.':tab==='Competition'?'Explore the shared traits connecting your game to its closest comparables.':'Explore regional interest for your comparable games. Pulsing markers represent sourced audience observations.'}</p></div>
        ...
        {tab==='Demographics' && <><div className="p-5 font-mono text-xs text-platinum/60"><p>Regional search interest · {year}. Scores are normalized separately for each game; they are not player counts.</p><button disabled={regionalQuery.isFetching||!competitors.length} onClick={()=>regionalQuery.refetch()} className="mt-3 text-moss disabled:opacity-40">{regionalQuery.isFetching?'Fetching live regional interest…':'Fetch regional interest ↻'}</button>{regionalQuery.isError&&<p role="alert" className="text-pulse">{regionalQuery.error.message}</p>}{game.regionalCoverage?.length>0 && <details className="mt-3"><summary className="cursor-pointer text-[10px]">Data coverage</summary>{game.regionalCoverage.map(c=><p key={c.competitor} className="mt-2 text-[10px]">{c.competitor}: {c.status}</p>)}</details>}</div><CompetitionGlobe games={regions} gameName={game.name} selected={selectedCountry} onSelect={setSelectedCountry}/><p className="font-mono text-[10px] text-platinum/40 px-5 pb-5">Country positions represent audience regions, not studio headquarters. Interest is not player count.</p></>}
      </section>
      <aside className="space-y-4">
        {tab!=='Demographics' && <section className={`p-5 bg-panel border border-white/10 ${tab==='Competition'?'cs-detail font-mono':''}`} aria-label={tab==='Competition'?'Competitor detail':'Genre fit'} aria-live="polite"><p className="font-mono text-[10px] text-pulse tracking-widest mb-4">{tab==='Genre'?'GENRE FIT':tab==='Competition'?'COMPETITOR DETAIL':'REGIONAL EVIDENCE'}</p>
          {tab==='Genre'&&(chosenGenre?<><h3 className="font-pixel text-sm text-moss mb-4">{chosenGenre.name}</h3><p className="font-mono text-xs leading-relaxed text-platinum/70">{chosenGenre.reason}</p><blockquote className="mt-5 border-l border-moss/40 pl-3 text-xs font-mono text-platinum/50">{chosenGenre.evidence}</blockquote><p className="font-mono text-[10px] text-platinum/35 mt-4">Relative fit {typeof chosenGenre.fit === 'number' ? chosenGenre.fit.toFixed(2) : '—'} / 1 · AI estimate, not measured demand.</p></>:<p className="font-mono text-xs leading-relaxed text-platinum/60">Genre analysis is not available for this game yet.</p>)}
        ...
        {tab==='Genre'&&genres.map((g,i)=><button key={g.name} onClick={()=>setSelectedGenre(g.name)} aria-pressed={chosenGenre?.name===g.name} className={`w-full p-4 text-left border font-mono text-xs ${chosenGenre?.name===g.name?'border-moss/50 text-moss':'border-white/10 text-platinum/60'}`}><span className="mr-3 text-platinum/30">0{i+1}</span>{g.name}<span className="block mt-2 text-platinum/40 text-[10px]">Relative fit {typeof g.fit === 'number' ? g.fit.toFixed(2) : '—'} / 1 · AI estimate{i===0?' · Closest':''}</span></button>)}
        {tab==='Demographics' && <p className="font-mono text-xs text-platinum/60">{regions.length?'Select a country to explore its games.':regionalQuery.isFetching?'Loading regional data…':'No regional measurements available for these games.'}</p>}
