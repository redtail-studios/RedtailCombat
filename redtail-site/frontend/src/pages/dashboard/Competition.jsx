...
export default function Competition() {
  ...
  return <GameWorkspace key={gameId || 'choose-game'} gameId={gameId}/>;
}
function GameWorkspace({gameId}) {
  ...
  if (!gameId) return <div className="max-w-[1500px] mx-auto p-4 sm:p-8"><MarketWelcome/>{portfolio.length > 0 && <section className="mt-8"><h2 className="font-pixel text-sm text-platinum mb-5">Your games</h2><div className="grid sm:grid-cols-2 gap-4">{portfolio.map(g => <Link key={g.id} to={`/dashboard/games/${encodeURIComponent(g.id)}`} className="p-6 bg-panel border border-white/10 hover:border-pulse/50 pixel-clip-sm"><span className="font-pixel text-sm text-platinum">{g.name}</span><span className="block font-mono text-xs text-moss mt-4">Open Competition ↗</span></Link>)}</div></section>}</div>;
  if (!game) return <div className="max-w-3xl mx-auto px-6 py-20 text-center"><Globe2 className="mx-auto w-12 h-12 text-pulse mb-8"/><h1 className="font-pixel text-xl text-platinum mb-5">{gameId ? 'Game not found' : 'Your game starts here.'}</h1><p className="font-mono text-sm text-platinum/50 leading-relaxed mb-8">{gameId ? 'Choose a saved game from your portfolio or upload a new game.' : 'Upload your game design document to create your Competition workspace and explore games in its genre.'}</p><Link className={button} to="/dashboard/analyze?tab=game"><Plus size={16}/>Add your game</Link><Link to="/dashboard/portfolio" className="block mt-6 font-mono text-xs text-platinum/60 underline">View portfolio</Link></div>;
  return <div className="max-w-[1500px] mx-auto p-4 sm:p-8 text-platinum">
    ...
    <header className="flex flex-wrap items-end justify-between gap-5 mb-8"><div><p className="font-mono text-[10px] text-moss tracking-[.2em] uppercase mb-3">YOUR GAME / MARKET INTELLIGENCE</p><h1 className="font-pixel text-xl sm:text-3xl leading-relaxed break-words">{game.name}<span className="text-pulse">_</span></h1></div><Link to="/dashboard/analyze?tab=game" className="font-mono text-xs border border-white/15 px-4 py-3 hover:border-pulse flex items-center gap-2"><Plus size={14}/>Add a game</Link></header>
    ...
    {section === 'competition' && query.data?.positioning && <section className="mb-6 p-5 border border-moss/20 bg-panel"><h2 className="font-pixel text-sm mb-3">Why would someone choose your game?</h2><p className="font-mono text-xs text-platinum/70 leading-relaxed">{query.data.positioning}</p><p className="font-mono text-[10px] text-platinum/40 mt-3">AI interpretation of {query.data.genreFit.document} · {query.data.genreFit.model} · {new Date(query.data.analysedAt).toLocaleString()}</p></section>}
    {section === 'competition' && (!query.isPending && !query.isError) && <CompetitionTabs regionalQuery={regionalQuery} key={`${game.id}:${year}`} game={enrichedGame} comparables={comparables} year={year} query={query} snapshot={snapshot}/>}
