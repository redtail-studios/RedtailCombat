...
from config import APP_STORE_APPS, APP_STORE_REVIEWS, get_year_dir
...
def run(year: int | None = None, log=print) -> list:
    log(f"[appstore] scraping {len(APP_STORE_APPS)} apps (year={year})")
    ...
    records = []
    for ap in APP_STORE_APPS:
        revs = _reviews(ap["app_id"], APP_STORE_REVIEWS)
        records.append({"source": "appstore", "app_id": ap["app_id"],
