...
from config import HN_QUERIES, HN_HITS_PER_Q, HN_COMMENTS_PER, get_year_dir
...
def run(year: int | None = None, log=print) -> list:
    log(f"[hackernews] scraping {len(HN_QUERIES)} queries (year={year})")
    ...
    for q in HN_QUERIES:
        ...
        for h in hits:
            ...
            try:
                ...
                pass
            records.append({
                "source": "hackernews", "id": oid, "title": title,
